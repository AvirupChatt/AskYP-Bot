...
public class main {
	...
	public static void main(String[] args) throws IOException{
	    ...
	    MongoCollection <Document> collectionBrands = dbsearch.db.getCollection("brands");
				...
				for (Status current : result.getTweets()) {
					...
					if (parsed_iteminfo.equals(item_info)) {
						...
						String keywords = tp.removeStopWords(msg);
				          keywords = keywords.replaceAll("[^a-zA-Z\\s]", "").toLowerCase();
				          keywords = keywords.replaceAll(" ", "+");
				          
						for (Status currentFav : fav) {
							String fav_msg = currentFav.getText();
				            resultBrands = collectionBrands.find(new Document("$text", new Document("$search", fav_msg).append("$caseSensitive", false).append("$diacriticSensitive", false))).iterator();
				            
				            if (resultBrands.hasNext()) {
				              String brand = resultBrands.next().toJson();
				              JsonObject gson = new Gson().fromJson(brand, JsonObject.class);
					...
					if (location_info.length() > 0) {
						...
						dbsearch.createTwitterUserObj(user, user.getLocation());
					} else {
						// no stored location
					...
					while (cursor.hasNext()) {
		                ...
		                Status tweet_ob = twitter.showStatus(Long.parseLong(saved_tweet.getString("tweet_id")));
		                //get replies
		                ArrayList<Status> replies = getDiscussion(tweet_ob,twitter);
		                for(Status stat : replies){
		                	if(stat.getUser().getScreenName().equals(tweet_ob.getUser().getScreenName())){
		                		// reply from user
		                		
		                		// update user location
		                		dbsearch.createTwitterUserObj(stat.getUser(), stat.getText());
		                		
		                		// post reply
		                		postTweetReply(stat.getId(), stat.getUser().getScreenName(), stat.getText(), (String) saved_tweet.get("item_info"));
